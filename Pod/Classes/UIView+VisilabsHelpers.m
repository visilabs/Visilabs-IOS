...
#define VISILABS_FINGERPRINT_VERSION 1

@implementation UIView (VisilabsHelpers)

- (int)visilabs_fingerprintVersion
{
    return VISILABS_FINGERPRINT_VERSION;
}
...
{
    UIImage *image = [self visilabs_snapshotImage];
    // hack, helps with colors when blurring
    NSData *imageData = UIImageJPEGRepresentation(image, 1); // convert to jpeg
    return [UIImage imageWithData:imageData];
}

// visilabs_targetActions
- (NSArray *)visilabs_targetActions
{
...
// Set by a userDefinedRuntimeAttr in the VisilabsTagNibs.rb script
- (void)setVisilabsViewId:(id)object
{
    objc_setAssociatedObject(self, @selector(visilabsViewId), [object copy], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSString *)visilabs_viewId
{
    return objc_getAssociatedObject(self, @selector(visilabsViewId));
}
#pragma clang diagnostic pop

- (NSString *)visilabs_controllerVariable
{
    NSString *result = nil;
    if ([self isKindOfClass:[UIControl class]]) {
 ...
 */
- (NSString *)visilabs_imageFingerprint
{
    NSString *result = nil;
...
static NSString* visilabs_encryptHelper(id input)
{
...
- (NSString *)visilabs_varA
{
    return visilabs_encryptHelper([self visilabs_viewId]);
}

- (NSString *)visilabs_varB
{
    return visilabs_encryptHelper([self visilabs_controllerVariable]);
}

- (NSString *)visilabs_varC
{
    return visilabs_encryptHelper([self visilabs_imageFingerprint]);
}

- (NSArray *)visilabs_varSetD
{
    NSArray *targetActions = [self visilabs_targetActions];
    NSMutableArray *encryptedActions = [NSMutableArray array];
    for (NSUInteger i = 0 ; i < [targetActions count]; i++) {
        [encryptedActions addObject:visilabs_encryptHelper(targetActions[i])];
    }
    return encryptedActions;
}

- (NSString *)visilabs_varE
{
    return visilabs_encryptHelper([self visilabs_text]);
}
